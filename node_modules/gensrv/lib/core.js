...
export class socket extends io
{
    ...
    connEvt(pSocket)
    {   
        console.log("Client connected - " + pSocket.conn.id)     
        pSocket.on('disconnect', () => 
        {
            //CLIENT DISCONNECT OLDUĞUNDA SQL BUFFER I TEMİZLENİYOR.
            for (let index = 0; index < this.core.sql.bufferList.filter(item => item.userId == pSocket.id).length; index++) 
            {
                this.core.sql.bufferList.splice(this.core.sql.bufferList.findIndex(item => item.userId == pSocket.id), 1);
            }
            ...
            {
                core.instance.log.msg('Client disconnected','Socket',pSocket.conn.id);
            }
        });
    }    
    clients()
...
export class sql 
{
    ...
    connEvt(pSocket)
    {
        ...
        pSocket.on('sql',async (pQuery,pCallback) =>
        {
            ...
            let tmpResult = await this.executePool(pQuery,pSocket.conn.id)
            
            if(typeof core.instance.config.debug != 'undefined' && core.instance.config.debug)
            {
                if(typeof tmpResult.result.err != 'undefined')
                {
                    core.instance.log.msg(JSON.stringify(pQuery) + '\n' + JSON.stringify(tmpResult),"Sql",pSocket.userInfo.CODE)
                }
                else
                {
                    core.instance.log.msg(JSON.stringify(pQuery),"Sql",pSocket.userInfo.CODE)
                }
            }
            //BUFFER DİZİSİNE SOCKET ID EKLENİYOR CLIENT DISCONNECT OLDUĞUNDA BU ID YE SAHİP BUFFERLAR TEMİZLENECEK.
            if(typeof tmpResult?.result?.bufferId != 'undefined')
            {
                let tmpBufIndex = this.bufferList.findIndex(item => item.id == tmpResult?.result?.bufferId)
                if(tmpBufIndex !== -1)
                {
                    this.bufferList[tmpBufIndex].userId = pSocket.id
                }
            }

            pCallback(tmpResult);
        });
        pSocket.on('sql_buffer',async (pParam,pCallback) =>
        {
            if(pSocket.authState().id != 0)
            {
                pCallback({auth_err:pSocket.authState()})
            ...
            let tmpBuf = this.bufferList.filter(x => x.id == pParam.bufferId);
            let tmpResult = {result:{recordset:[]}}

            if(tmpBuf.length > 0 && tmpBuf[0].result.length > 0)
            {
                tmpResult.result.recordset = tmpBuf[0].result.slice(pParam.start,pParam.end > tmpBuf[0].result.length ? tmpBuf[0].result.length : pParam.end)
            }
            pCallback(tmpResult);
        });
        pSocket.on('sql_buffer_remove',async (pParam,pCallback) =>
        {
            const tmpIndex = this.bufferList.findIndex(item => item.id == pParam.bufferId);
            if (tmpIndex !== -1) 
            {
                this.bufferList.splice(tmpIndex, 1);
            }
        });
        pSocket.on('disconnect',() => 
        {
            this.removePool(pSocket.conn.id)
        });
    }
    async addPool(pId)
    {
        let tmpPool = await this.getPool(pId)
        if(typeof tmpPool == 'undefined')
        {
            try
            {
                tmpPool = await this.connect()
                this.poolList.push({id:pId,pool:tmpPool})
            }
            catch(err)
            {
                console.log("connEvt sql error : 980 " + err)
            }
        }
    }
    async getPool(pId,pConfig)
    {
        return new Promise(async resolve =>
        {
            try
            {
                let tmpPool = this.poolList.find(pool => pool.id === pId)

                if(typeof pConfig?.database != 'undefined' && typeof tmpPool?.pool?.config?.database != 'undefined' && tmpPool?.pool?.config?.database != pConfig.database)
                {
                    let tmpConfig = 
                    {
                        server : tmpPool.pool.config.server,
                        database : tmpPool.pool.config.database,
                        uid : tmpPool.pool.config.user,
                        pwd : tmpPool.pool.config.password,
                        connectionTimeout : tmpPool.pool.config.connectionTimeout,
                        requestTimeout : tmpPool.pool.config.requestTimeout,
                        trustedConnection : tmpPool.pool.config.options.trustedConnection
                    }
                    tmpConfig.database = pConfig.database
                    this.removePool(pId)
                    let tmpConPool = await this.connect(tmpConfig)
                    tmpPool = {id:pId,pool:tmpConPool}
                    this.poolList.push(tmpPool)
                }
                resolve(tmpPool?.pool)
            }
            catch(err)
            {
                console.log("connEvt sql error : 981 " + err)
                resolve()
            }
        })
    }
    removePool(pId)
    {
        const tmpIndex = this.poolList.findIndex(pool => pool.id === pId);
        if (tmpIndex !== -1) 
        {
            this.poolList[tmpIndex].pool.close().then(() => 
            {}).catch(err => 
            {
                console.error(`Error closing pool with id ${pId}:`, err);
            });
            this.poolList.splice(tmpIndex,1);
        }
        return
    }
    executePool(pQuery,pId)
    {
        return new Promise(async resolve =>
        {
            try
            {   
                ...
                {
                    tmpConfig.database = pQuery[0].db
                }

                let tmpPool = await this.getPool(pId,tmpConfig)
                //TRANSACTION - 25.07.2021
                if(Array.isArray(pQuery))
                {
                    const transaction = new mssql.Transaction(tmpPool);
                ...
                else
                {
                    ...
                    request.query(pQuery.query,(err,result) => 
                    {
                        if(err == null)
                        {
                            //QUERY SONUCU BUFFER A ALINIYOR CLIENT sql_buffer CAĞIRISYLA GELEN DATAYI PARCA PARCA ALABİLİYOR.
                            if(typeof pQuery.buffer != 'undefined' && pQuery.buffer)
                            {
                                let tmpId = crypto.randomBytes(16).toString('hex')
                                this.bufferList.push({id:tmpId,result:result.recordset})

                                resolve({tag : pQuery.tag,result : {bufferId : tmpId,count : result.recordset.length}});
                            }
                            else
                            {
    ...
    async connect()
    {
        return new Promise(async (resolve, reject) =>
        {
            ...
            let tmpPool = new mssql.ConnectionPool(tmpConfig);
            ...
            {
                console.error("Error errCode : 105 - " + err, "Sql");
    ...
    execute(pQuery)
    {
        return new Promise(resolve =>
        {
            ...
            {   
                ...
                let TmpConfig = this.connection();
                let tmpQueryDb = pQuery.db
                if(Array.isArray(pQuery) && pQuery.length > 0)
                {
                    tmpQueryDb = pQuery[0].db
                }
                
                if (typeof tmpQueryDb != "undefined") 
                {
                    if(typeof tmpQueryDb == 'string')
                    {
                        TmpConfig.database = tmpQueryDb
                    }
                    else
                    {
                        TmpConfig = this.connection(tmpQueryDb);
                    }
                }
    ...
    inputBuild(pQuery)
    {
        if(typeof pQuery.param != 'undefined')
        {
            for(let i = 0;i < pQuery.param.length;i++)
            {
                ...
                if(pType[0] == "string")
                {
                    this.input(pQuery.param[i].split(":")[0],mssql.NVarChar(pType[1]),String(pQuery.value[i]));
                }
                else if(pType[0] == "int")
                {
...
export class auth
{
    ...
    connEvt(pSocket)
    {
        ...
        {    
            ...
            {
                TmpApp = pParam[1];
                TmpResult = await this.login(pParam[0],pParam[2]);
            ...
            else if(pParam.length == 4)
            {
                TmpApp = pParam[2];
                TmpResult = await this.login(pParam[0],pParam[1],pParam[3]);
        ...
        pSocket.on('getUserList',async (pParam,pCallback) =>
        {    
            let TmpResult = [];
            TmpResult = await this.getUserList(pParam);
            pCallback(TmpResult);
        });
    ...
    getUserList()
    {
        return new Promise(async resolve =>
        {
            let TmpQuery =
            {
                query:  "SELECT CODE,NAME,CARDID,(SELECT CAST( CAST( PWD as XML ).value('.','varbinary(max)') AS varchar(max) )) AS PWD,ROLE,USER_APP FROM USERS WHERE STATUS = 1",
            }
            
            if(arguments.length > 0 && arguments[0] != '')
            {
                TmpQuery.db = arguments[0]
            }
...
export class util 
{
    ...
    {        
        pSocket.on('util',async (pParam,pCallback) =>
        {
            ...
            else if(pParam.cmd == 'version')
            {
                pCallback(this.getVersion())
            }
        })
    }
    folder_list(pPath)
    ...
    getVersion()
    {
        try
        {
            let appInfo = JSON.parse(fs.readFileSync(core.instance.root_path + '/package.json', 'utf8'))
